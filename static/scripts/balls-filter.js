document.addEventListener('DOMContentLoaded', () => {
    const ballCards = Array.from(document.querySelectorAll('.ball-card'));
    ...
    function getFilteredCards() {
      ...
      return ballCards.filter(card => {
        ...
        const matchSize = selectedSize ? cardSize === selectedSize : true;
        const matchLevel = selectedLevel ? cardLevel === selectedLevel : true;
  
        return matchBrand && matchStock && matchPrice && matchSize && matchLevel;
      });
    }
    ...
    // Разворачивающиеся фильтры (+/-)
    document.querySelectorAll('.filter-header').forEach(header => {
      header.addEventListener('click', () => {
        const content = header.nextElementSibling;
        content.classList.toggle('hidden');
        const toggleSymbol = header.querySelector('.toggle');
